...
class Postac:
    ...
    def talenty(self, tal):

        blyskot = ('Błyskotliwość', 'Int', 0)
        aptek = ('Aptekarz', 'Int', 0)
        return tal

    def tal_maks(self, tal):
        """Funkcja oblicza bonus z cechy przypisanej do talentu."""
        if tal[3] is int:
            return tal[3]
        else:
            return self.umiej(tal)[0] // 10

    blyskot = ('Błyskotliwość', 'Int', 0, None)
    #aptek = ('Aptekarz', 0, maks, 'Int')
...
prop_rasa = los_rasy(95)  # temp zawsze krasnolud
...
print(p.tal_maks(p.blyskot))  # temp
print('test')  # temp
"""Wprowadzone zmiany:
- Dokumentacja wybranych funkcji.
- Obsługa umiejętności grupowych"""
